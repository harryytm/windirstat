...
void CTreeListControl::ExpandItem(const int i, const bool scroll)
{
    ...
    for (int c = 0; c < childItems; c++)
    {
        ...
        // LargeFileCount, to ensure all items displayed on
        // the large file list have the correct column width
        // to show the entire file path
        if (COptions::AutomaticallyResizeColumns && scroll && c < min(max(COptions::LargeFileCount,50),1000))
        {
