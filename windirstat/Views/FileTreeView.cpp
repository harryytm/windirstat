...
void CFileTreeView::CreateColumns(const bool all)
{
    if (all)
    {
        ...
        m_Control.InsertColumn(CHAR_MAX, Localization::Lookup(IDS_COL_NAME).c_str(), LVCFMT_LEFT, 260, COL_NAME);
    ...
    if (COptions::ShowColumnItems)
        m_Control.InsertColumn(CHAR_MAX, Localization::Lookup(IDS_COL_ITEMS).c_str(), LVCFMT_RIGHT, 65, COL_ITEMS);
    if (COptions::ShowColumnFiles)
        m_Control.InsertColumn(CHAR_MAX, Localization::Lookup(IDS_COL_FILES).c_str(), LVCFMT_RIGHT, 65, COL_FILES);
    if (COptions::ShowColumnFolders)
        m_Control.InsertColumn(CHAR_MAX, Localization::Lookup(IDS_COL_FOLDERS).c_str(), LVCFMT_RIGHT, 65, COL_FOLDERS);
    if (COptions::ShowColumnLastChange)
        m_Control.InsertColumn(CHAR_MAX, Localization::Lookup(IDS_COL_LASTCHANGE).c_str(), LVCFMT_LEFT, 105, COL_LASTCHANGE);
    if (COptions::ShowColumnAttributes)
        m_Control.InsertColumn(CHAR_MAX, Localization::Lookup(IDS_COL_ATTRIBUTES).c_str(), LVCFMT_LEFT, 75, COL_ATTRIBUTES);
    if (COptions::ShowColumnOwner)
        m_Control.InsertColumn(CHAR_MAX, Localization::Lookup(IDS_COL_OWNER).c_str(), LVCFMT_LEFT, 150, COL_OWNER);
